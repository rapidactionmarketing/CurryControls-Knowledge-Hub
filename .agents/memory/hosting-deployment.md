...
The mark is a white "C" with one light-blue control pulse leaving through its opening, on the brand navy with a faint blueprint grid. The source of truth is `artifacts/currycontrols/public/favicon.svg` (modern browsers use it at every size). The raster files beside it are derived from that SVG by rendering it in headless Chromium at each pixel size, not drawn separately: `favicon.ico` (a PNG-in-ICO container with 16, 32 and 48 px entries; the 16 px entry is a simplified C with a single blue contact square, because the pulse turns to mush at that size), `apple-touch-icon.png` (180 px, square and full bleed because iOS applies its own corner mask), `icon-192.png` and `icon-512.png` (rounded, for the manifest), and `icon-512-maskable.png` (full bleed with the artwork scaled to 78% so it survives Android's circular mask). `site.webmanifest` lists the manifest icons and the theme colour. `index.html` links all of them, and the prerender step keeps those links on every route. If the mark changes, edit the SVG and regenerate the rasters the same way; do not hand-edit the PNGs. The `.htaccess` in `public/` declares the SVG, ICO and manifest MIME types for the cPanel host.
